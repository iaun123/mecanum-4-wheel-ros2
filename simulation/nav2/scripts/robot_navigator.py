...
class BasicNavigator(Node):
    ...
    def goToPose(self, pose): # Sends a `NavToPose` action request
        ...
        self.info('Navigating to goal: ' + str(pose.pose.position.x) + ' ' +
                    str(pose.pose.position.y) + ' ' + str(yaw))
    ...
    def followWaypoints(self, poses):  # Sends a `FollowWaypoints` action request
    ...
    def getPath(self, start, goal): # Sends a `NavToPose` action request
    ...
    def _waitForNodeToActivate(self, node_name): # Waits for the node within the tester namespace to become active
